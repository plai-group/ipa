...
if __name__ == "__main__":
    ...
    wandb.init(project=PROJECT_NAME, entity=os.environ['WANDB_ENTITY'],
               config=args, tags=["train_structure_generator"] + args.tags,
    ...
    # NOTE: Masks here are inverted. Mask shape: [1, H, W, 1]
    ...
    def sample_from_model(sess, dataset):
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for i in range(args.max_steps):
            ...
            if (i + 1) % 100 == 0:
                l_nll = np.mean(train_nll[-100:])
                tm = time.time() - begin
                print('%d iterations, time: %ds, train NLL: %.5f.' % 
                    ((i + 1), tm, l_nll))
                sys.stdout.flush()
                wandb.log({"loss/rec": l_nll,
                           "time/iter": tm,
                           "step": i+1})
                begin = time.time()
            ...
            if (i + 1) % args.val_steps == 0:
                ...
                vl_nll = np.mean(valid_nll)
                print('%d iterations, time: %ds, valid NLL: %.5f.' % 
                    ((i + 1), time.time()-begin, vl_nll))
                sys.stdout.flush()
                wandb.log({"validation/NLL": vl_nll,
                           "step": i+1})
                begin = time.time()
            ...
            if (i + 1) % args.train_spe == 0:
                ...
                print('%d iterations, generation time: %.3fs.' % ((i + 1), tm))
                sys.stdout.flush()
